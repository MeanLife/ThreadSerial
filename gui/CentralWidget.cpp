...
/*-----------------------------------------------------------------------------|
 |                                CentralWidget                                |
 |----------------------------------------------------------------------------*/
CentralWidget::CentralWidget(QWidget *parent) : QWidget(parent), ui(new Ui::CentralWidget), d(new CentralWidgetPrivate(this)) ,isSerialOpened(false){
    ...
    settings = new SerialPortSettingsDialog;
    ...
    connect(this, &CentralWidget::sOpenSerialPortToWork, ctrlWork, &Controller::openSerialPort);
    connect(this, &CentralWidget::sCloseSerialPortToWork, ctrlWork, &Controller::closeSerialPort);
...
void CentralWidget::openSerialPort()
{
    emit sCloseSerialPortToWork();
    g_SerialSetting = settings->settings();
    serial->setPortName(g_SerialSetting.name);
    serial->setBaudRate(g_SerialSetting.baudRate);
    serial->setDataBits(g_SerialSetting.dataBits);
    serial->setParity(g_SerialSetting.parity);
    serial->setStopBits(g_SerialSetting.stopBits);
    serial->setFlowControl(g_SerialSetting.flowControl);
    if (serial->open(QIODevice::ReadWrite)) {
        ...
        QString str=tr("Connected to %1 : %2, %3, %4, %5, %6")
                .arg(g_SerialSetting.name).arg(g_SerialSetting.stringBaudRate).arg(g_SerialSetting.stringDataBits)
                .arg(g_SerialSetting.stringParity).arg(g_SerialSetting.stringStopBits).arg(g_SerialSetting.stringFlowControl);
...
void CentralWidget::closeSerialPort()
{
    emit sCloseSerialPortToWork();
    console->setEnabled(false);
    ui->itemButton16->setEnabled(true);
...
void CentralWidget::handleError(QSerialPort::SerialPortError error)
{
...
{
    //    connect(ui->itemButton16, &QAction::triggered, this, &CentralWidget::openSerialPort);
    //    connect(ui->itemButton17, &QAction::triggered, this, &CentralWidget::closeSerialPort);
